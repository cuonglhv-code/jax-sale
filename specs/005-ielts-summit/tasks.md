...
- [X] T004 Define summit domain types — Placement discriminated union (measured with optional
...
- [X] T005 [P] Content-module validation test: Zod-parse pricing/copy/brand modules (known
...
- [X] T006 [P] Zod boundary schemas — summitRequestSchema (targetBand > currentBand refinement),
...
- [X] T007 [P] [US1] Engine unit tests: exhaustive all-band-pair contiguity property (climb is a
...
- [X] T008 [P] [US1] Pricing unit tests: total = arithmetic sum of climb stage prices for the
...
- [X] T009 [P] [US1] Timeline unit tests: weeks = sessions/2.7, months = weeks/4.33, output is
...
- [X] T010 [US1] Implement pure summit engine (generateSummitRoadmap: shared 002 slice logic +
...
- [X] T011 [US1] Opening controls (student name, current band, target band; summit current-band
...
- [X] T012 [P] [US1] Mountain scene (layered SVG/CSS, bottom-to-top always, illuminate climb /
...
- [X] T013 [P] [US1] Stage panel (one-at-a-time accordion; tier shapes per FR-009 —
...
- [X] T014 [P] [US1] Summary surface (total buổi, duration range, projected finish window,
      total price + excludesUnpriced note, Pre-S flexible note) in
      src/app/(app)/lo-trinh-ielts/SummarySurface.tsx
- [X] T015 [US1] Summit shell — state map with every state one action from every other
...
- [X] T016 [US1] UI behaviour tests: exactly one stage expanded at a time, DOM stage order is
...
- [X] T017 [P] [US2] Mode rendering tests: estimated placement renders the exact caveat string
...
- [X] T018 [P] [US2] Structural tests: renderer components accept only the Placement union
...
- [X] T019 [US2] Mode selection in opening (Mode A with test date / Mode B) + record-placement-
      result flow flipping B→A, in src/app/(app)/lo-trinh-ielts/OpeningControls.tsx (evolve)
- [X] T020 [US2] Provisional treatment across the scene: distinct start-marker rendering in
      Mountain.tsx, estimate framing in SummarySurface.tsx, caveat + CTA placement in
      Summit.tsx (all copy from summit-copy.ts)
- [X] T021 [US2] PDF cover takes the Placement union; the estimated branch IS the prominent
...
- [X] T022 [P] [US3] Migration for summit_sends (columns per data-model.md incl. optional
...
- [X] T023 [P] [US3] Integration tests against local Supabase (real DB, no mocks — house rule):
...
- [X] T024 [P] [US3] PDF content tests: section order exactly cover → timeline → course cards →
...
- [X] T025 [US3] Evolve the PDF document to the summit shape (spec section order, climb
...
- [X] T026 [US3] Review screen: preview from the same SummitRoadmap the PDF consumes, inline
...
- [X] T027 [US3] Capture form (student email/phone, consultant name/phone/email; Zod
      captureSchema; centre from verified claims, never client input) in ReviewSend.tsx +
      src/schemas/summit.ts wiring
- [X] T028 [US3] Send pipeline: EmailSendAdapter behind the 002 DeliveryAdapter seam in
...
- [X] T029 [US3] Loud-failure UX: blocking Vietnamese error, PDF blob + all state preserved,
      one-action retry, download+maildraft fallback (002 adapter) — in ReviewSend.tsx
- [X] T030 [US3] Reset flow: one obvious action from everywhere, warns on prepared-but-unsent
      document, clears all PII to blank state — in Summit.tsx
- [X] T031 [US3] Academic audit listing: list-sent-roadmaps action
...
- [X] T032 [P] [US4] Proof content module with consent brand: raw entries internal, the ONLY
...
- [X] T033 [P] [US4] Proof tests: matchProof ranks by |startΔ|+|resultΔ| with editorial
...
- [X] T034 [US4] Implement pure matchProof in src/services/ielts/proof-match.ts — T033 green
- [X] T035 [US4] Summit proof surface (arrival treatment as state style, matched proof first,
...
- [X] T036 [P] [US5] FAQ content module (objectionKey, chipLabelVi, questionVi, answerVi,
      priority; ≤ ~8 visible chips, editorial order) in src/lib/domain/ielts/faq.ts
- [X] T037 [P] [US5] Secondary-content tests: commitments component renders both thresholds
...
- [X] T038 [US5] Secondary content surface (persistent rail; ecosystem view from ecosystem.ts;
...
- [X] T039 [P] Minimal service worker precaching the summit shell, JS/CSS, fonts, brand assets
      for cold-start offline (research D-OFFLINE) in public/sw.js + registration in
      src/app/(app)/lo-trinh-ielts/page.tsx
- [X] T040 [P] Guard tests: "Olympia" appears nowhere in src/ or content data (Principle VIII);
...
- [X] T041 Diacritics & brand pass: full Vietnamese diacritics at every used weight/size on
...
- [X] T042 Offline validation: quickstart Scenario 3 full walk (session offline + cold start via
      SW; send failure preserves work; retry without re-entry)
- [X] T043 Coverage gate ≥ 80% (npm run test:cov) + full quickstart Scenarios 1–7 pass +
      content-edit test (price/FAQ/narrative changes touch only lib/domain/ielts/)
- [X] T044 Code review + security review of the send/archive surface (permission gate, RLS,
