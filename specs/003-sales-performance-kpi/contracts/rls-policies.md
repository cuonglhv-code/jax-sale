# Contract: RLS Policies (`*_kpi_rls.sql`)

Authoritative tenancy for the KPI tables. Claims read as subquery-cached `(select auth.jwt() ->>
'...')` (foundation research R2). `personal_kpis` uses the **new tiered-read** pattern (D-TIER);
`department_kpi_targets` uses Pattern B (network reference, admin write); status logs are append-only.
...
-- ── personal_kpis — TIERED read (own → centre → network); centre-narrow write ──
...
create policy "pkpi_select_tiered" on public.personal_kpis for select to authenticated
  using (
    ...
    -- teacher: no branch ⇒ no rows
  );

-- INSERT: a consultant creates their OWN row in their OWN centre (seeded on first record).
create policy "pkpi_insert_own" on public.personal_kpis for insert to authenticated
  with check (
    ...
    and (
      (select auth.jwt() ->> 'app_role') in ('centre_manager', 'centre_admin')   -- managers may seed target rows
      or (
        (select auth.jwt() ->> 'app_role') = 'sale_consultant'
        and consultant_id::text = (select auth.jwt() ->> 'employee_id')
      )
...
-- UPDATE: confined to own centre (column-level limits imposed by enforce_actual_only trigger +
-- the guarded approve/reject functions). Consultants further limited to their own row.
...
-- ── department_kpi_targets — Pattern B: broad-ish read (managers+admins), super_admin write ──
...
## Isolation proofs these policies must pass (→ tests, SC-003/004/006)

- A `sale_consultant` SELECT returns only rows with `consultant_id = self`; a raw select of a peer's
  row returns 0 rows.
- A `centre_manager` of centre A cannot SELECT/UPDATE a centre-B row (0 rows / RLS rejection),
  including via the guarded functions (they run as invoker).
- A non-`super_admin` INSERT/UPDATE on `department_kpi_targets` is rejected.
- A `teacher` SELECT on `personal_kpis` returns 0 rows.
