"use server";

import { listApprovalQueueCore } from "@/services/hr-request.service";
import { createServerSupabaseClient } from "@/lib/supabase/server";
import { assertPermission } from "@/lib/auth/assert-permission";
import { withError, type ActionResult } from "@/lib/server-action";
import type { HrRequest } from "@/lib/data/types";

/** US2 (contracts/hr-requests.actions.md — useApprovalQueue): the manager's decision queue. */
export async function listApprovalQueue(): Promise<ActionResult<HrRequest[]>> {
  return withError(async () => {
    const supabase = await createServerSupabaseClient();
    const claims = await assertPermission(supabase, "hrRequest.decide");
    return listApprovalQueueCore(supabase, claims);
  });
}
